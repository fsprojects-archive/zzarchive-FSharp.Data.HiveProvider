...
type ProvidedTypeDefinition(container:TypeContainer,className : string, baseType  : Type option) as this =
    ...
    member this.AddMembersDelayed(makeMS : unit -> list<#MemberInfo>) =
        membersQueue.Add (fun () -> 
            let members = 
                try makeMS() |> List.map (fun x -> x :> MemberInfo)
                with ex ->
                    let createNote isStatic name = 
                        let note = ProvidedProperty(name, typeof<bool>, IsStatic = isStatic, GetterCode = fun _ -> <@@ false @@>)
                        note.AddXmlDoc(ex.Message)
                        note :> MemberInfo
                    [createNote true "note"; createNote false "note'"]
            members
            |> List.map (fun x -> patchUpAddedMemberInfo this x; x ))
...
type TypeProviderForNamespaces(namespacesAndTypes : list<(string * list<ProvidedTypeDefinition>)>) =
    ...
    let parametricCache = Dictionary HashIdentity.Structural
...
//        | None -> null
    ...
    interface System.IDisposable with 
        member x.Dispose() = x.Dispose()//AppDomain.CurrentDomain.remove_AssemblyResolve handler
#endif

    member __.AddNamespace (namespaceName,types:list<_>) = otherNamespaces.Add (namespaceName,types)
    member self.Invalidate() = 
        parametricCache.Clear()
        invalidateE.Trigger(self,EventArgs())
    interface ITypeProvider with
        ...
        override this.ApplyStaticArguments(ty,typePathAfterArguments:string[],objs) = 
            ...
            match ty with
            | :? ProvidedTypeDefinition as t ->
                match parametricCache.TryGetValue typePathAfterArguments with
                | true, pt -> pt
                | false, _ ->
                    let pt = (t.MakeParametricType(typePathAfterArguments,objs) :> Type)
                    parametricCache.Add(typePathAfterArguments, pt)
                    pt
